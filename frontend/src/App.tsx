...
export default function App() {
  ...
  const [dragging, setDragging] = useState<'vertical' | 'horizontal' | 'composer' | null>(null)
  ...
  const [pdfStatus, setPdfStatus] = useState<'idle' | 'updating' | 'done'>('idle')
  ...
  async function uploadResume(file: File) { setBusy(true); setError(''); const form = new FormData(); form.append('file', file); try { const p = await apiJson('/api/import/resume', { method: 'POST', body: form }); setResume(p); setStructured(p.parsed.structured ?? null); setAssets(p.parsed.assets ?? []); setResumeVersionId(''); setViewMode('edit'); setPageImages([]); setPdfDirty(false); setOriginalFileUrl(`${API_BASE}/api/resume/file?t=${Date.now()}`); const template = await apiJson('/api/template'); if (template.config) setTemplateConfig(template.config) } catch (e) { setError(e instanceof Error ? e.message : '简历导入失败') } finally { setBusy(false) } }
  ...
  async function generateResume() {
    ...
    try { const p = await apiJson('/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jd_text: jdText, history: chat }) }); setStructured(p.content); setResumeVersionId(p.version_id); setVersionName(p.display_name ?? '当前岗位版本'); setViewMode('edit'); setPageImages([]); setPdfDirty(true); setCanUndo(false); const analysis = p.match_analysis ?? {}; const summary = [`实际匹配：${analysis.actual_match || '待结合材料判断'}`, `简历呈现：${analysis.resume_match || '已按JD调整'}`, `主要修改：${analysis.changes || '强化相关经历并压缩弱相关内容'}`, `材料缺口：${analysis.gaps || '未发现明确缺口'}`].join('\n'); const note = p.review_notes?.length ? `\n复核：${p.review_notes.join(' ')}` : ''; setChat((items) => [...items, { role: 'assistant', content: `已生成当前岗位版本。\n${summary}${note}` }]) }
    catch (e) { setError(e instanceof Error ? e.message : '简历生成失败') } finally { setBusy(false) }
  }
  async function saveStructured(next: ResumeStructured) { setStructured(next); if (!resumeVersionId) return; setPdfDirty(true); try { const p = await apiJson(`/api/versions/${resumeVersionId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: next }) }); setStructured(p.content); setCanUndo(Boolean(p.can_undo)) } catch (e) { setError(e instanceof Error ? e.message : '修改保存失败') } }
  async function undoVersion() { if (!resumeVersionId) return; try { const p = await apiJson(`/api/versions/${resumeVersionId}/undo`, { method: 'POST' }); setStructured(p.content); setCanUndo(false); setPdfDirty(true) } catch (e) { setError(e instanceof Error ? e.message : '撤销失败') } }
  async function generatePdf(versionId: string) {
    setPdfStatus('updating')
    const url = `${API_BASE}/api/versions/${versionId}/preview.png?t=${Date.now()}`
    setPageImages([url])
    setPdfStatus('done')
    return { url }
  }
  async function ensureLatestPdf() {
    if (!resumeVersionId) return ''
    if (!pdfDirty && pageImages.length) return pageImages[0]
    const result = await generatePdf(resumeVersionId)
    setPdfDirty(false)
    return result.url
  }
  async function switchToPdfView() { await ensureLatestPdf(); setViewMode('pdf') }
  async function downloadPdf() {
    if (!resumeVersionId || busy) { setError('请先生成岗位版本。'); return }
    setBusy(true); setError('')
    try {
      const url = await ensureLatestPdf(); if (!url) throw new Error('请先生成岗位版本。')
      const p = await apiJson(`/api/versions/${resumeVersionId}/export`, { method: 'POST' })
      const file = await fetch(`${API_BASE}/api/versions/${resumeVersionId}/pdf`); if (!file.ok) throw new Error('PDF 下载失败')
      const blobUrl = URL.createObjectURL(await file.blob()); const a = document.createElement('a'); a.href = blobUrl; a.download = p.filename ?? `${versionName || 'resume'}.pdf`; a.click(); URL.revokeObjectURL(blobUrl)
      const allowedPages = preferences.page_limit === 'one' ? 1 : preferences.page_limit === 'two' ? 2 : Infinity
      if (p.page_count > allowedPages) setChat((items) => [...items, { role: 'assistant', content: `导出的PDF为${p.page_count}页，超过你设置的页数限制，请压缩内容或调整版式。` }])
      if (!preferences.profile_text) { try { const suggestion = await apiJson('/api/preferences/suggest', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ version_id: resumeVersionId }) }); setPreferenceDraft(suggestion.suggestion ?? ''); setShowPreferenceConfirm(true) } catch { /* PDF export remains successful */ } }
  ...
  function startDrag(kind: 'vertical' | 'horizontal' | 'composer', event: ReactPointerEvent) { event.preventDefault(); setDragging(kind) }
  ...
  return <main className="shell">
    ...
    <section className="workspace" ref={workspaceRef}><aside className="left-column" style={{ flexBasis: `${leftWidth}%` }}>
      ...
      <section className="panel chat-panel"><div className="panel-heading"><span>AI 协作</span><span className="muted">{chat.length} 条消息</span></div><div className="chat-body">{resume && <div className="system-note">已载入主简历：{resume.filename}</div>}{jdText && <div className="system-note">已载入 JD：{jdText.length} 字</div>}{chat.length === 0 ? <p className="chat-placeholder">可直接生成，也可以先告诉 AI 你的要求。</p> : chat.map((item, i) => <div className={item.role === 'user' ? 'user-message' : 'assistant-message'} key={`${item.role}-${i}`} style={{ whiteSpace: 'pre-wrap' }}>{renderPlain(item.content)}</div>)}</div><div className="splitter horizontal composer-splitter" onPointerDown={(event) => startDrag('composer', event)} /><div className="composer" style={{ height: `${composerHeight}px` }}><textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendMessage() } }} placeholder={resumeVersionId ? '继续告诉 AI 如何修改当前版本……' : '告诉 AI 你的要求……'} /><button className="send-button" onClick={() => void sendMessage()} disabled={busy}>发送</button></div></section>
    </aside><div className="splitter vertical" onPointerDown={(event) => startDrag('vertical', event)} /><section className="preview-panel"><div className="panel-heading preview-heading"><span className="preview-title">{resumeVersionId ? versionName || '当前岗位版本' : '原始简历预览'}</span><div className="view-toggle"><button className={viewMode === 'edit' ? 'active' : ''} onClick={() => setViewMode('edit')} disabled={!resumeVersionId}>编辑</button><button className={viewMode === 'pdf' ? 'active' : ''} onClick={() => void switchToPdfView()} disabled={!resumeVersionId}>PDF效果</button></div><div className="preview-actions">{canUndo && <button className="ghost-button" onClick={() => void undoVersion()}>撤销</button>}<button className="ghost-button" onClick={() => void generateResume()} disabled={busy}>{resumeVersionId ? '重新生成' : '生成简历'}</button><button className="primary-button" onClick={() => void downloadPdf()} disabled={!resumeVersionId || busy}>下载 PDF</button></div></div><div className="resume-placeholder">{!resumeVersionId ? (resume?.format === 'pdf' && originalFileUrl ? <iframe className="original-pdf" src={originalFileUrl} title="原始简历 PDF" /> : <ResumePaper structured={structured} assets={assets} template={templateConfig} editable={false} onChange={() => undefined} />) : (viewMode === 'edit' ? <ResumePaper structured={structured} assets={assets} template={templateConfig} editable onChange={(next) => void saveStructured(next)} /> : <div className="pdf-preview">{pdfStatus === 'updating' ? <div className="pdf-status">正在更新PDF效果……</div> : <>{pageImages.map((src, index) => <img key={index} src={src} alt={`第 ${index + 1} 页`} />)}<div className="pdf-status">PDF效果已更新</div></>}</div>)}</div></section></section>
