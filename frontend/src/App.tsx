...
export default function App() {
  ...
  async function uploadResume(file: File) { setBusy(true); setError(''); const form = new FormData(); form.append('file', file); try { const p = await apiJson('/api/import/resume', { method: 'POST', body: form }); setResume(p); setStructured(p.parsed.structured ?? null); setAssets(p.parsed.assets ?? []); setResumeVersionId(''); setPreviewUrl(''); setOriginalFileUrl(`${API_BASE}/api/resume/file?t=${Date.now()}`); const template = await apiJson('/api/template'); if (template.config) setTemplateConfig(template.config) } catch (e) { setError(e instanceof Error ? e.message : '简历导入失败') } finally { setBusy(false) } }
