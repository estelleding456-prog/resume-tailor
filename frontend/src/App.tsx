...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/resume`).then((r) => r.json()).then((p) => { if (p.exists) { setResume(p); setStructured(p.parsed.structured ?? null); setAssets(p.parsed.assets ?? []); setOriginalFileUrl(`${API_BASE}/api/resume/file?t=${Date.now()}#toolbar=0&navpanes=0&view=FitH`) } }).catch(() => undefined)
  ...
  async function uploadResume(file: File) { setBusy(true); setError(''); const form = new FormData(); form.append('file', file); try { const p = await apiJson('/api/import/resume', { method: 'POST', body: form }); setResume(p); setStructured(p.parsed.structured ?? null); setAssets(p.parsed.assets ?? []); setResumeVersionId(''); setOriginalFileUrl(`${API_BASE}/api/resume/file?t=${Date.now()}#toolbar=0&navpanes=0&view=FitH`); const template = await apiJson('/api/template'); if (template.config) setTemplateConfig(template.config) } catch (e) { setError(e instanceof Error ? e.message : '简历导入失败') } finally { setBusy(false) } }
